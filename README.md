...
- Rust downstream build helpers are provided by the Cargo crate in `rust-build/`.
...
For downstream protobuf or gRPC generation, add the companion build helper as a
build dependency and map the MATSim protobuf package to the Rust types exported
by `matsim-schemas`:
...
matsim_schemas_build::configure_extern_path(&mut config);

let include_paths = [
    std::path::PathBuf::from("proto"),
    matsim_schemas_build::proto_dir(),
];
...
- `matsim_schemas_build::PROTO_PACKAGE`
- `matsim_schemas_build::RUST_TYPES_PATH`
- `matsim_schemas_build::proto_dir()`
- `matsim_schemas_build::proto_files()`
- `matsim_schemas_build::configure_extern_path(...)`
